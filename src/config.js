...
module.exports = (action,k,v) => {
  ...
  if(action === 'get'){
    if (flag) {
      ...
      console.log(`没有此项，您可能是想输入为 ${chalk.green('asqy-cli config get <k>')}命令，\n比如:  ${chalk.green('asqy-cli config get org')}`);
    }   
  } else if(action === 'set'){
    if( k || v){
      obj.k = 'k';
      ...
      console.log(`没有此项，您可能是想输入为 ${chalk.green('asqy-cli config set <k> <v></v>')}命令，\n比如:  ${chalk.green('asqy-cli config set org lxy-cli')}`);
    }
